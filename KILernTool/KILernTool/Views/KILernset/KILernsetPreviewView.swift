...
struct KILernsetPreviewView: View {
    ...
    // MARK: - Theo Preview Banner

    private var theoPreviewBanner: some View {
        TheoPreviewMessage(cardCount: cards.count)
    }
...
private struct TheoPreviewMessage: View {
    ...
    private var text: String { fullText.replacingOccurrences(of: "{count}", with: "\(cardCount)") }

    var body: some View {
        HStack(alignment: .top, spacing: 14) {
            MascotView(color: accent, mood: .happy, size: 56)
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(accent.opacity(0.07))
                .overlay(RoundedRectangle(cornerRadius: 18).stroke(accent.opacity(0.18), lineWidth: 1))
        )
        .task { await typeText() }
    }
    ...
    private func typeText() async {
        ...
        while idx < full.endIndex {
            let step = min(3, full.distance(from: idx, to: full.endIndex))
